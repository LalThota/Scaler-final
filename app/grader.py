...
def _clamp_open_interval(score: float) -> float:
    return max(EPSILON, min(UPPER_BOUND, score))
...
def calculate_action_confidence(base_score_dict: Dict[str, float], action: Action, task: Task) -> float:
    ...
    ambiguity_penalty = 0.1 if (task.ambiguity_level == "high" and action.ask_clarification) else 0
    confidence = (0.75 * base_score) + (0.25 * action_completeness) + ambiguity_penalty
    return max(EPSILON, min(UPPER_BOUND, confidence))
...
def calculate_base_score(action: Action, task: Task) -> Dict[str, float]:
    ...
    priority_score = UPPER_BOUND if action.priority.lower() == task.expected_priority.lower() else EPSILON
    ...
    response_score = EPSILON
    if action.response_message:
        keywords_matched = 0
        for kw in task.ground_truth_response_keywords:
            if kw.lower() in action.response_message.lower():
                keywords_matched += 1
        response_score = keywords_matched / len(task.ground_truth_response_keywords) if task.ground_truth_response_keywords else UPPER_BOUND
    
    # 0.1 resolution and escalation score
    resolution_score = EPSILON
